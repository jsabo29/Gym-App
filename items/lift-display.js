...
export default function LiftDisplay({index, sets, reps, weight, movement, date, onDelete}){
  const [xIsPressed, changeXIsPressed] = useState(false)
  return(
    <View style={styles.mainContainer}>
      <Text style={[styles.data, {width: 200}]}>{movement}</Text>
      <Text style={[styles.data, {width: 90}]}>{sets}x{reps}</Text>
